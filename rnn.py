...
def layer_norm_all(h,
  ...
  h_reshape = tf.reshape(h, tf.convert_to_tensor((-1, base, num_units)))
  ...
  h = tf.reshape(h_reshape, tf.convert_to_tensor((-1, base * num_units)))
...
class LayerNormLSTMCell(tf.contrib.rnn.RNNCell):
  ...
  def __call__(self, x, state, timestep=0, scope=None):
    with tf.variable_scope(scope or type(self).__name__):
      ...
      if self.use_recurrent_dropout:
        noise = tf.random.uniform((self.num_units, self.num_units), dtype=h.dtype)
        keep_mask = tf.cast(noise <= self.dropout_keep_prob, tf.float32)
        w_hh_ = tf.split(w_hh, 4, 1)
        w_hh_ = [hh*keep_mask for hh in w_hh_]
        w_hh = tf.concat(w_hh_, axis=1)
  ...
  def __call__(self, x, state, timestep=0, scope=None):
    with tf.variable_scope(scope or type(self).__name__):
      ...
      if self.use_recurrent_dropout:
        ...
        #w_hh = tf.nn.dropout(w_hh, self.dropout_keep_prob) * self.dropout_keep_prob
      """
